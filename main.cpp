...
int randint(int lb, int ub) {
  ...
  int ret = lb - 1;
  while (ret < lb || ret > ub) {
    ret = round(normal_distribution<>{(ub - lb) / 2, (ub - lb) / 6}(gen));
  }
  return ret;
}
const int WIDTH = 800;
...
int main(int argv, char** args) {
  ...
  FC_LoadFont(font, renderer, "sources/minu.ttf", 60,
              FC_MakeColor(0, 200, 0, 255), TTF_STYLE_NORMAL);
  FC_LoadFont(h2, renderer, "sources/minu.ttf", 40,
              FC_MakeColor(255, 255, 255, 255), TTF_STYLE_NORMAL);
  FC_LoadFont(h3, renderer, "sources/minu.ttf", 22,
              FC_MakeColor(255, 255, 255, 255), TTF_STYLE_NORMAL);
  FC_LoadFont(p, renderer, "sources/minu.ttf", 16,
              FC_MakeColor(255, 255, 255, 255), TTF_STYLE_NORMAL);
  ...
  while (running) {
    ...
    int rgb = (int)max(min(255, 255 - (int)(abs(sin(time * 1.2)) * 127)), 0);
    char* msg = "按任意鍵繼續";
  ...
  while (running) {
    while (SDL_PollEvent(&event)) {
      if (event.type == SDL_QUIT) {
        ...
        if (event.key.keysym.sym == SDLK_r) {
          ...
          speed = 0;
          timer = 30;
          height_interval = 40.0;
        } else if (event.key.keysym.sym == SDLK_KP_PLUS) {
          if (timer)
            timer = timer - 10 < 0 ? 0 : timer - 10;
          else if(speed < 1000)
            speed = min(10000, speed + 10);
          else if(speed < 10000)
            speed = min(10000, speed + 100);
        } else if (event.key.keysym.sym == SDLK_KP_MINUS) {
          if (speed == 0)
            timer = timer + 10 > 300 ? 300 : timer + 10;
          else
          ...
          help = !help;
                   ...
                   SDL_GetTicks() - screenshot_timer > 1000) {
    ...
    FC_DrawAlign(p, renderer, 100, 550, FC_ALIGN_CENTER, "%d", -1);
    FC_DrawAlign(p, renderer, 400, 550, FC_ALIGN_CENTER, "%d", 0);
    FC_DrawAlign(p, renderer, 700, 550, FC_ALIGN_CENTER, "%d", 1);
    FC_DrawAlign(p, renderer, 80, 535, FC_ALIGN_CENTER, "%d", 0);
    FC_DrawAlign(p, renderer, 80, 150, FC_ALIGN_CENTER, "%d",
                 (int)(400.0 / height_interval));
    FC_DrawAlign(p, renderer, 80, 342, FC_ALIGN_CENTER, "%d",
                 (int)(400.0 / height_interval) / 2);
    ...
    FC_DrawAlign(h3, renderer, 10, 60, FC_ALIGN_LEFT, "%s",
                 con ? "暫停中" : "模擬中");
    FC_DrawAlign(h3, renderer, 12, 90, FC_ALIGN_LEFT, "%.2f 倍速",
                 timer ? 30.0 / timer : max(4.0f, float(speed)));

    if (!con) {
      int flag = 1;
      if (speed) flag = speed + (rand() % (speed / 8 + 1)) - (rand() % (speed / 8 + 1));
